...
<main class="bg-purple-100 font-nunito text-black/[.87] flex flex-col items-center gap-10 pt-10 pb-60">
  ...
  <p class="max-w-prose">In Hanamikoji, two players compete to earn the favor of seven Geisha by collected the performance items the Geisha desire.</p>
  <button
    on:click={() => startMatch()}
    disabled={isLoading}
  ...
  <section class="max-w-prose pb-2">
    ...
    <p>This is the blue Geisha Card and the corresponding blue Item Card. The Item Cards for each Geisha share the Geisha's color and number.</p>
    <p>The number on the Item Card also represents how many of that card are in the deck e.g. there are 3 blue Item Cards in the deck.</p>
  </section>
  <section class="max-w-prose pb-2">
    ...
    <div class="flex flex-col gap-2 pt-1">
      ...
      <p>A deck of 20 Item Cards (There are 21 Item Cards in total, but one Item Card is removed from the deck at random at the beginning of each round.)</p>
  ...
  <section class="max-w-prose pb-2">
    ...
    <div class="flex flex-col gap-2 pt-1">
      ...
      <p>Then for each Geisha, compare the number of Item Cards each player has played on their side of the board. If one player has more Item Cards, move the Victory Marker for that Geisha to that player's side.</p>
      <p>A Victory Marker on a player's side means that that player wins the Geisha and gains Charm Points equal to the number on the Geisha Card.</p>
      <p>After determining the position of each Victory Marker, calculate the number of Geisha and Charm Points each player has.</p>
      <p>If no player has won, reset the deck and start another round.</p>
      <p>Keep in mind that Victory Markers remain in place, so during the next round's scoring phase, a player will win a Geisha if the Victory Marker is already on that player's side and the number of Item Cards is tied.</p>
    </div>
  </section>
  <section class="max-w-prose pb-2">
    ...
    <div class="flex flex-col gap-2 pt-1">
      ...
      <p>If one player wins 4 Geisha and the other has 11 or more Charm Points, the player with 11 or more Charm Points wins the game.</p>
    </div>
  </section>
</main>
